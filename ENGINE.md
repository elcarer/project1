...
ячеек в пул `freeArrays` → `insert()` по `getCellKey(x,y)`. Ключ уникален для любых
координат (схема `(col+32768)*65536 + (row+32768)`, диапазон ±1 млн пикселей) — коллизий
ключей нет даже при отрицательных координатах; обратный разбор — `getCellCoords(key)`.
...
пара сущностей проверяется ровно один раз, а число обращений к карте упало с 9 на
сущность до 5 на ячейку (−19% времени кадра на нагруженной сцене). Дистанция — сравнение
квадратов; физика пары вынесена в `resolveCollision(a, b)`.
...
| Сущностей | До culling | Culling | Culling + парные коллизии + WebGPU (объекты ×0.5) |
|---|---|---|---|
| 5 061 | 144 FPS / 6.94 мс | 144 FPS / 6.95 мс | 144 FPS / 6.95 мс |
| 15 061 | 62 FPS / 17.4 мс | ~70 FPS / ~15 мс | 85 FPS / 12.6 мс |
| 25 061 | 32.5 FPS / 31.4 мс | ~40 FPS | 43 FPS / 24.3 мс |

Порог «без лагов» (≥60 FPS): **~20–21 тыс.** объектов (18k → 66 FPS, 21k → 59.4 FPS).
