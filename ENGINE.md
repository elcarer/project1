...
**Открытие**: имя файла + «Открыть» — редактор читает PNG и манифест с сервера и
сам разбивает картинку на строки-анимации и кадры (проверено round-trip:
открыть → экспорт → открыть — пиксели совпадают). Манифест обязателен: PNG без
него не открывается.
